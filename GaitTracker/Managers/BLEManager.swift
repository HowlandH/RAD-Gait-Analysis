    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            case strideLengthCharUUID:
                ...
                self.metrics.strideLength = value
                self.metrics.distance += value
                self.metrics.stepCount += 1

            case cadenceCharUUID:
                let value = data.withUnsafeBytes { $0.load(as: UInt16.self) }
                self.metrics.cadence = Int(value)

            case strikeTypeCharUUID:
                let value = data.withUnsafeBytes { $0.load(as: UInt8.self) }
                self.metrics.strikeType = StrikeType(rawValue: value) ?? .midfoot

            case batteryCharUUID:
                let value = data.withUnsafeBytes { $0.load(as: UInt8.self) }
                self.metrics.batteryLevel = Int(value)
